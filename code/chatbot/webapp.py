...
def _repo_root() -> Path:
    # webapp.py lives at: <repo>/code/chatbot/webapp.py
    return Path(__file__).resolve().parents[2]


# Load environment from repo-root .env if present (helps local dev / setx not required)
...
PERSIST_DIR = os.getenv("PERSIST_DIR", str(_repo_root() / "code" / "chatbot" / "vector_data"))
...
def _workspace_root() -> Path:
    # Backwards-compatible alias: in this repo, "workspace" means repository root.
    return _repo_root()
