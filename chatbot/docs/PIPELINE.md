...
Run the commands in this document from the `chatbot` directory unless a section explicitly says to use the repository root.
...
1. Start the Colab server from `../notebooks/colab_llm_server.ipynb` and copy the printed `LLM_API_BASE` URL.
2. In PowerShell set the environment variable locally (or add it to `chatbot/.env`):
...
The helper script [scripts/extract_test.py](../scripts/extract_test.py) samples markdown chunks, runs a mock extractor (or the real Groq extractor), and writes JSONL records to an output file. Run it from the `chatbot` directory.
